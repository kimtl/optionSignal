...
| `OPTIONSIGNAL_OTM_POINTS=150` | 서버 기본 CPPI(headline) 범위: 현재가 ±N포인트(콜·풋 같은 범위, 내가격·등가격 포함). 비우면 체인의 모든 행사가. 화면의 「범위」는 이 값과 무관하게 브라우저별로 고릅니다 |
...
- **콜/풋 프리미엄 합**: 개별 옵션을 고르는 대신 체인의 **모든 행사가**(내가격·등가격·외가격 전부)를 **마지막 체결가 × 거래량**으로 합산합니다. 카드에 콜 합 `Σ(체결가×거래량)`(빨강), 풋 합(파랑), CPPI(노랑), 콜/풋 비율과 합산된 행사가 개수가 나오고, 선물 봉차트 오른쪽 축에 콜 합·풋 합이 선으로 겹쳐집니다. 합산 범위는 CPPI 「범위」(기본 모든 행사가)를 따릅니다.
...
- **화면별 설정**: CPPI 「범위」(모든 행사가 / 지수 ±100·150·200·300포), CPPI 선 표시, 분봉·시간 범위는 모두 브라우저(localStorage)에만 저장되어 다른 사람의 조작에 영향받지 않습니다. 서버는 틱마다 모든 범위의 CPPI를 함께 계산해(`cppi_variants`) 내려 주고, 화면이 자기 범위로 카드·테이프·선을 그립니다. 지수 선택(NQ/ES/YM)도 탭마다 따로입니다(아래 「지수 선택」 참고). **모든 기준은 모든 행사가**입니다: 내가격·등가격·외가격을 가리지 않고 콜과 풋이 같은 행사가 범위를 보며, 가격은 중간가가 아니라 **마지막 체결가**(DXLink `Trade.price`, REST `last`/`mark`, Yahoo `lastPrice`; 아직 체결이 없는 계약만 중간가)를 거래량에 곱합니다. 「±N포」는 콜·풋 모두 현재가−N~현재가+N. 예: 지수 100, ±20이면 콜 80~120, 풋 80~120. 「모든 행사가」는 서버가 구독한 0DTE 체인(등가격 ±8%, 최대 250개 행사가) 전부를 합산합니다. 헤드라인 CPPI(`headline_*`)와 만기별 슬라이스도 같은 기준입니다.
- **CPPI 선**: 선물 봉차트 안에 선택한 범위의 CPPI가 노란 선으로 겹쳐집니다. 눈금은 표시 구간의 CPPI 고저에 맞춰 자동으로 잡히고(왼쪽 위/아래에 `CPPI 고`/`CPPI 저`), 0이 구간 안에 있으면 점선으로 0선을 그립니다. 차트 헤더의 「CPPI 선」 버튼으로 끄고 켭니다. 콜 합 선(빨강)과 풋 합 선(파랑)도 각자 표시 구간의 고저에 맞춘 자기 눈금으로 그려져 차트 높이를 모두 쓰며, **풋 합 선은 상하 반전**(풋 프리미엄이 늘면 아래로)이라 콜 선과 같은 방향으로 읽을 수 있습니다. 오른쪽에 `콜 합 고/저`, `풋 합 고/저`가 표시됩니다(두 선의 높이는 서로 비교하지 말고 각자의 흐름으로 보세요).
- **콜/풋 프리미엄 비율 차트**: 선물 차트 바로 아래 봉차트. 「범위」 선택과 무관하게 항상 **체인의 모든 행사가 합계**(틱의 `cppi_variants.all`; 예전에 저장된 틱은 `chain`)로 `콜 Σ(체결가×거래량) ÷ 풋 Σ(체결가×거래량) × 100`을 틱마다 계산해 1분/5분봉으로 묶습니다(100 = 콜·풋 프리미엄 흐름이 같음, 노란 점선). 값은 틱의 `cppi_variants`에서 바로 계산하므로 별도 요청이 없습니다. 특정 계약만 골라 비율을 보고 싶으면 `GET /api/premium_ratio?calls=24700C,24750C&puts=24700P&tf=1`을 쓸 수 있습니다.
...
- **CPPI**(콜·풋 프리미엄 불균형)와 「범위」 메뉴, 1분 변화, RR, 테이프는 참고용으로 접힌 섹션에 남겨 두었습니다.
