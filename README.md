...
- **콜/풋 프리미엄 비율 차트**: 선물 차트 바로 아래 봉차트. 「선정」과 무관하게 **체인의 모든 행사가 합계**(내가격·등가격 포함, 틱의 `cppi_variants.chain`)로 `콜 Σ(중간가×거래량) ÷ 풋 Σ(중간가×거래량) × 100`을 틱마다 계산해 1분/5분봉으로 묶습니다(100 = 콜·풋 프리미엄 흐름이 같음, 노란 점선). 값은 틱의 `cppi_variants`에서 바로 계산하므로 별도 요청이 없습니다. 특정 계약만 골라 비율을 보고 싶으면 `GET /api/premium_ratio?calls=24700C,24750C&puts=24700P&tf=1`을 쓸 수 있습니다.
