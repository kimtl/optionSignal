...
- **화면별 설정**: 옵션 선택, CPPI 「선정」(모든 행사가 / 등가격 ±100·150·200·300포), CPPI 선 표시, 분봉·시간 범위는 모두 브라우저(localStorage)에만 저장되어 다른 사람의 조작에 영향받지 않습니다. 서버는 틱마다 모든 선정 규칙의 CPPI를 함께 계산해(`cppi_variants`) 내려 주고, 화면이 자기 선정값으로 카드·테이프·선을 그립니다. **지수 선택(NQ/ES/YM)만 서버 공통**입니다(피드가 하나라서). 「±N포」는 콜과 풋이 **똑같은 행사가 범위**(현재가−N ~ 현재가+N, 내가격 포함)를 보고 합산합니다. 예: 지수 100, ±20이면 콜·풋 모두 80~120. 「모든 행사가」는 서버가 구독한 0DTE 체인(등가격 ±8%, 최대 250개 행사가) 안에서 호가가 있는 콜·풋 전부를 합산합니다.
